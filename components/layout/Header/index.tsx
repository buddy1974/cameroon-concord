...
function WorldCupNavLink({ pathname }: { pathname: string }) {
  const { worldCup } = siteSpecials
  if (!worldCup.active || !worldCup.showInNav) return null
  const active = pathname === worldCup.href || pathname.startsWith(`${worldCup.href}/`)
  return (
    <Link
      href={worldCup.href}
      className="link-underline"
      style={{
        fontSize: '1rem', fontWeight: 700, textDecoration: 'none', whiteSpace: 'nowrap',
        transition: 'color 0.2s', fontFamily: 'var(--font-roboto)',
        color: active ? 'hsl(133 61% 65%)' : 'hsl(133 61% 50%)',
        display: 'inline-flex', alignItems: 'center', gap: 5,
      }}
    >
      <span>&#x26BD;</span>
      <span>{worldCup.label}</span>
    </Link>
  )
}
    ...
    <header style={{
      ...
      <div className="glass shadow-elegant" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '32px', minWidth: 0 }}>
          <Link
            href="/"
            aria-label="Cameroon Concord"
            style={{ display: 'inline-flex', alignItems: 'center', textDecoration: 'none', flexShrink: 0 }}
          >
            <span style={{
              background: 'hsl(210 20% 95%)', color: 'hsl(222 15% 7%)',
              padding: '3px 8px', fontFamily: 'var(--font-fraunces)', fontWeight: 900,
              fontSize: 'clamp(0.65rem, 2.5vw, 0.82rem)', letterSpacing: '0.06em', lineHeight: 1.4,
            }}>CAMEROON</span>
            <span style={{
              background: 'hsl(354 78% 50%)', color: '#fff',
              padding: '3px 8px', fontFamily: 'var(--font-fraunces)', fontWeight: 900,
              fontSize: 'clamp(0.65rem, 2.5vw, 0.82rem)', letterSpacing: '0.06em', lineHeight: 1.4,
            }}>CONCORD</span>
          </Link>
          <nav className="hidden lg:flex" style={{ alignItems: 'center', gap: '24px' }}>
            ...
            {/* World Cup tab -- controlled by lib/site-specials.ts */}
            <WorldCupNavLink pathname={pathname} />
          </nav>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
          ...
          <Link
            href="/#newsletter"
            className="hidden md:inline-flex animate-glow-pulse"
            style={{
              background: 'hsl(354 78% 50%)', color: '#fff',
          ...
          <button
            ...
            style={{
              ...
              color: '#fff', fontSize: '1.1rem', padding: '4px', lineHeight: 1,
            }}
          >
            {mobileOpen ? '✕' : '☰'}
          </button>
      ...
      {mobileOpen && (
        <div
          className="glass animate-fade-up lg:hidden shadow-elegant"
          style={{ maxWidth: '1400px', margin: '8px auto 0', borderRadius: '16px', padding: '20px' }}
        >
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '16px' }}>
            {NAV_CATEGORIES.map(cat => (
              <Link
                ...
                style={{
                  ...
                  background: 'hsl(220 14% 10%)', color: 'hsl(220 8% 72%)',
            ...
            {/* World Cup -- mobile */}
            {siteSpecials.worldCup.active && siteSpecials.worldCup.showInNav && (
              <Link
                href={siteSpecials.worldCup.href}
                onClick={() => setMobileOpen(false)}
                style={{
                  padding: '10px 14px', borderRadius: '10px',
                  background: 'hsl(133 61% 8%)', color: 'hsl(133 61% 55%)',
              ...
              >
                <span>&#x26BD;</span>
                <span>{siteSpecials.worldCup.label} Special</span>
              </Link>
            )}
          </div>
          <Link
              ...
              {[...breaking, ...breaking].map((a, i) => (
                <span key={i} style={{ display: 'inline-flex', alignItems: 'center', gap: '10px' }}>
                  <span style={{
                    ...
                    color: 'hsl(354 78% 55%)', letterSpacing: '0.15em',
                  ...
                  <span style={{ color: 'hsl(220 8% 35%)', fontSize: '0.65rem', flexShrink: 0, margin: '0 16px 0 6px' }}>
                    &#xB7;
                  </span>
                </span>
              ))}
